...
QList<int> ScriptV4::syntaxErrorsLines(){
   return QList<int>{0}; 
}

QStringList ScriptV4::syntaxErrorsLinesString()
{
    ScriptRunner *runner = new ScriptRunner(doc(), m_data);
...
QString ScriptV4::convertLine(const QString& str, bool *ok)
{
    ...
    while (left < line.length())
    {
        ...
        else
        {
            ...
            else if (command == Script::waitCmd)
            {
...
{
    if (method == Script::startFunctionCmd) return "Engine.startFunction";
    else if (method == Script::stopFunctionCmd) return "Engine.stopFunction";
    else if (method == Script::blackoutCmd) return "Engine.setBlackout";
    else if (method == Script::waitCmd) return "Engine.waitTime";
    else if (method == Script::setFixtureCmd) return "Engine.setFixture";
    else if (method == Script::systemCmd) return "Engine.systemCommand";
    else return "";
}
